...
## The intervention reference

The seven framings above examine the *observation*. The document below examines the candidate *intervention* referenced in the preprint and the public-health framing — that is, what was done to the subject during the period when the bimodal fraction collapsed.

### `The_beat_of_Nightbheat.docx` — companion note to the overnight tone script

A short, plain-language description of the parameter-controlled audio signal used in subsequent characterisation work: a sinusoidal carrier whose frequency steps down in 5 Hz increments every 9 minutes, with each step containing 20-second cos²-shaped inner cycles (~3 cycles per minute, deliberately slower than the ~5 bpm resonance-breathing target so that drifting toward resonance feels effortless rather than forced). Includes a frequency-versus-time visualisation, run instructions for Windows, and the full `nightbheat.py` script reproduced inline.

**Important scope note:** this document and the accompanying script describe the *synthetic, parameter-controlled reproduction* of the mechanical signal characterised in the preprint, intended for the controlled-jig experiment described under "Next instrumentation step" below — not the original mechanical configuration (a Meaco 1056P pedestal fan coupled to an aluminium element) that produced the mid-December 2025 collapse in the n=1 record. The synthetic version exists to make the signal reproducible, parametrically variable, and shareable, so that the carrier-Q and envelope contributions can be separated experimentally rather than left bundled in a fixed mechanical artefact.

## What ties the framings together

All seven framings converge on the same single ask: run a binary, falsifiable analytical pilot on existing wearable data using the specified GMM detection pipeline, and either confirm that REM-respiration bimodality is a real population-scale phenotype or rule it out. They differ in audience and in which downstream pathway they sketch beyond that test, but the test itself is shared, fully specified, and computationally cheap. A negative result is informative and worth publishing. A positive result opens distinct downstream possibilities — product, clinical, occupational, public-health — that the individual framings then explore.

The intervention reference sits outside this convergence on purpose. It does not strengthen or weaken the case for running the population-scale detection pilot, which stands on the observation regardless of how the intervention worked or whether it generalises. It is included so that a reader who reaches the preprint's effect-size comparison and asks *"what exactly was the signal?"* has a self-contained answer in the same repository.
...
The synthetic intervention reference (`The_beat_of_Nightbheat.docx` + `nightbheat.py`) is the controllable input side of that instrumentation step: with the pillow sensor on the readout side and a parametrically variable signal on the drive side, the same jig can probe whether carrier-Q, envelope shape, or audibility of the cardiac-modulation channel is the load-bearing variable — an experiment the fixed mechanical configuration cannot perform.

## Status and limitations

Single-subject. Single-investigator. Subject–experimenter overlap. Screenshot-based scoring rather than raw-sample GMM fitting. No formal washout completed. No measured blood pressure data in the n=1 record. No cross-subject replication. No direct glymphatic measurement. The intervention reference describes the synthetic reproduction of the signal, not the original mechanical configuration, and has not itself been tested against the bimodality endpoint in a controlled comparison. Each document declares its own limitations section; read those.
